...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $existssql1 = "select * from `signup` where phone = '$phone' ";
    $result1 = mysqli_query($conn, $existssql1);
    $numexistsrows = mysqli_num_rows($result);
    $numexistsrows1 = mysqli_num_rows($result1);
    if ($numexistsrows > 0) {
        ...
        if ($numexistsrows1 > 0) {
            $showerror = " Phone no. already exists";
        } else {
            if (($password1 == $cpassword)) {

                $sql = "INSERT INTO `signup` (`name`, `phone`, `email`, `password`, `date & time`) VALUES ('$name', '$phone', '$email', '$password1', CURRENT_TIMESTAMP)";
